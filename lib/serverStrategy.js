...
module.exports = function(options, requestOptions, id, cb) {
    ...
    ensurePhantom(phantom, function(err) {
        ...
        phantom.execute(requestOptions, function (err, res) {
            ...
            var stream = fs.createReadStream(requestOptions.output);

            if(requestOptions.cleanTmpFiles) {
                ['headerFile','htmlFile','footerFile'].forEach(function (file) {
                    if(typeof requestOptions[file] !== 'undefined') {
                        fs.unlinkSync(requestOptions[file]);
                    }
                });
                stream.on('end', function() {
                    fs.unlinkSync(requestOptions.output);
                })
            }
